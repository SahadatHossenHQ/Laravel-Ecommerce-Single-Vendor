...
<style>
  ...
  <section class="hero-area" style="margin-top:2px">
        <div class="container">
            <div class="row">
               <div class="dtrr"></div>
                <div class="hero-slider col-lg-9">
                    <div class="slideshow">
    ...
    <div class="products camppp">
        <div class="container">
           <div class="row" style="margin:-10px;margin-bottom:10px;background: white;border-radius: 5px;padding: 10px;">
                <h3 class="title col-md-12" style="margin: 0 !important;border: none;"><span style="">{{$campain->name}} Sell</span> : <span  style="background: #ec0313;padding: 8px;color: white;" class="ends" id="demo{{$campain->id}}"></span> <a href="{{route('campaing.product',['slug'=>$campain->slug])}}">View All</a></h3>
           </div>
                <script>
                    // Set the date we're counting down to
                    var countDownDate{{$campain->id}} = new Date("<?php echo $mdate=$campain->end?>").getTime();

                    // Update the count down every 1 second
                    var x{{$campain->id}} = setInterval(function() {

                      // Get today's date and time
                      var now{{$campain->id}} = new Date();
                      // Find the distance between now and the count down date
                      var distance{{$campain->id}} = countDownDate{{$campain->id}} - now{{$campain->id}};

                      // Time calculations for days, hours, minutes and seconds
                      var days{{$campain->id}} = Math.floor(distance{{$campain->id}} / (1000 * 60 * 60 * 24));
                      var hours{{$campain->id}} = Math.floor((distance{{$campain->id}} % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
                      var minutes{{$campain->id}} = Math.floor((distance{{$campain->id}} % (1000 * 60 * 60)) / (1000 * 60));
                      var seconds{{$campain->id}} = Math.floor((distance{{$campain->id}} % (1000 * 60)) / 1000);

                      // Display the result in the element with id="demo"
                      document.getElementById("demo{{$campain->id}}").innerHTML = days{{$campain->id}} + "d " + hours{{$campain->id}} + "h "
                      + minutes{{$campain->id}} + "m " + seconds{{$campain->id}} + "s ";

                      // If the count down is finished, write some text
                      if (distance{{$campain->id}} < 0) {
                        clearInterval(x{{$campain->id}});
    ...
    <div class="product">
        ...
        <div class="product-wrapper"  @if(setting('is_point')==1) style="height: 310px;" @endif>
            <div class="pin">
                ...
                <a href="{{route('product.cam.details', $cam_products->id)}}">
                 ...
                 <div class="details" style="padding-top:0 !important">
                   ...
                   <h6><strong style="color: #108b3a">৳{{$cam_products->price ?? $product->discount_price ?? $product->regular_price}}</strong> 
                    @if($cam_products->price>0 || $product->discount_price>0)
                   <del>৳{{$product->regular_price}}</del></h6>
                   @endif
                    
                </div>
                <div class="quick-view">  <a href="{{route('product.cam.details', $cam_products->id)}}"><i class="icofont icofont-search"></i> Quick View</a></div>
            </div>
            @php
             $hw=App\Models\wishlist::where('product_id', $product->id)->where('user_id',auth()->id())->first();
                    if($hw){
         ...
         <a  href="{{route('product.details', $product->slug)}}" class="redirect" style="margin-top: 10px;background: red;color: white;border-color: red;" >Pre Order </a>
            @else
                    <button type="submit" class="redirect" style="margin-top: 10px;" data-url="{{route('camp.product.info', $cam_products->id)}}" id="productInfo1" type="submit" title="Add To Cart"><i class="fal fa-shopping-cart" aria-hidden="true"></i> </button>
                    @endif
...
<div class="products">
    <div class="container">
        <h3 class="title"><span>{{$cat->name}} </span><a href="{{route('category.product',$cat->slug)}}">View All</a></h3>
        <div class="row autoplay slick-slides">
             @forelse ($products as $product)
...
<div class="products">
    <div class="container">
        <h3 class="title"><span>{{$cat->name}}</span>  <a href="{{route('subCategory.product',$cat->slug)}}">View All</a></h3>
        <div class="row autoplay slick-slides">
             @forelse ($products as $product)
                <x-product-grid-view :product="$product" classes="" />
            @empty
...
<div class="products">
    <div class="container">
        <h3 class="title"><span>{{$cat->name}} </span> <a href="{{route('miniCategory.product',$cat->slug)}}">View All</a></h3>
        <div class="row autoplay slick-slides">
             @forelse ($products as $product)
                <x-product-grid-view :product="$product" classes="" />
            @empty
...
<div class="products">
    <div class="container">
        <h3 class="title"><span>{{$cat->name}}</span> <a href="{{route('extraCategory.product',$cat->slug)}}">View All</a></h3>
        <div class="row autoplay slick-slides">
             @forelse ($products as $product)
                <x-product-grid-view :product="$product" classes="" />
            @empty
...
<div class="shop-category shop-brand" style="padding-bottom: 20px;text-align: center;">
    <div class="container">
        <h3 class="title" style="margin-bottom: 0 !important;"><span>Shop By Brands</span>  <a href="/brands/list">view all</a></h3>
...
We'll never share your email address with a third-party</h6>
                       ...
                       <form action="{{route('subscription')}}" method="Post" id="subs">
                            ...
                            <div class="input-group">
                                ...
                                <button style="width:initial" class="input-group-addon components-bg" type="submit">Subscribe </button>
                            </div>
                        </form>
                       </div>
